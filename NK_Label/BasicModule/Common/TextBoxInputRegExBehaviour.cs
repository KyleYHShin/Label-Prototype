...
namespace BasicModule.Common
{
    ...
    public class TextBoxInputRegExBehaviour : Behavior<TextBox>
    {
        ...
        void PreviewTextInputHandler(object sender, TextCompositionEventArgs e)
        {
            ...
            e.Handled = !ValidateText(text); // Cannot restrict other language (Ex. Korean)
            //if (e.Handled && text.Length > 0)
            //{
            //    TextBox txtBox = sender as TextBox;
            //    string change = new Regex(this.RegularExpression, RegexOptions.IgnoreCase).Replace(text, string.Empty);
            //    txtBox.Text = text;
            //    txtBox.Focus();
    ...
    public class TextBoxNameBehaviour : Behavior<TextBox>
    {
        private Regex regex = new Regex("^[a-zA-Z0-9_-]+$", RegexOptions.IgnoreCase);
        private string antiPattern = @"[ㄱ-ㅎㅏ-ㅣ가-힣\s~!@#$%^&*()+=\[{\]};:<>|./?,\\'""]";
        ...
        void PreviewTextInputHandler(object sender, TextCompositionEventArgs e)
        {
            ...
            {
                TextBox txtBox = sender as TextBox;
                txtBox.Text = Regex.Replace(text, antiPattern, string.Empty, RegexOptions.None); ;
                txtBox.Focus();
                txtBox.SelectionStart = txtBox.Text.Length;
            }
        }
    ...
    public class TextBoxNumberBehaviour : Behavior<TextBox>
    {
        private Regex regex = new Regex("^[0-9]+$", RegexOptions.None);
        private string antiPattern = @"[ㄱ-ㅎㅏ-ㅣ가-힣a-zA-Z\s!@#$%^&*()_+=\[{\]};:<>|./?,\\'""~-]";
    ...
    public class TextBoxDenyKoreanBehaviour : Behavior<TextBox>
    {
        private Regex regex = new Regex(@"^[0-9a-zA-Z\s!@#$%^&*()_+=\[{\]};:<>|./?,\\'""~-]+$", RegexOptions.None);
        private string antiPattern = @"[ㄱ-ㅎㅏ-ㅣ가-힣]";
