...
{
    public static class ZebraImage
    {

        public static string GetASCIIhexadecimalData(Bitmap bmpSource)
        {
            if (bmpSource == null)
            {
            ...
            using (var bmpCompressed = bmpSource.Clone(dim, PixelFormat.Format1bppIndexed))
            {
                ...
                result.AppendFormat("^GFA,{0},{0},{1},", stride * dim.Height, stride);
        ...
        #region Print uploaded image in printer buffer

        // How to use in code
        //var baseStream = new MemoryStream();
        //var tw = new StreamWriter(baseStream, Encoding.UTF8);
        //tw.WriteLine(ZebraImage.GetGrfStoreCommand("R:LBLRA2.GRF", bitmap));
        //tw.WriteLine(ZebraImage.GetGrfPrintCommand("R:LBLRA2.GRF"));
        //tw.WriteLine(ZebraImage.GetGrfDeleteCommand("R:LBLRA2.GRF"));
        ...
        #endregion Print uploaded image in printer buffer
        ...
        unsafe private static byte[][] GetImageData(Rectangle dim, int stride, Bitmap bmpCompressed)
        {
            ...
            try
            {
                data =  bmpCompressed.LockBits(dim, ImageLockMode.ReadOnly, PixelFormat.Format1bppIndexed);
    ...
    public class GdiPrintJob
    {
        ...
        public void WritePage(Stream data)
        {
            ...
            {
                UInt32 written;
                if (!NativeMethods.WritePrinter(PrinterHandle, buffer, read, out written))
                    throw new Win32Exception();
